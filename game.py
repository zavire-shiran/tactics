...
movewindowspec = [0.32, 0.19,
                  ...
                  ['CT', 0.210, 0.005, 0.035],
                  ['***stats.ct', .265, 0.005, 0.035],
                  ...
                  ['@Wait', 0.210, 0.120, 0.035]]

movedwindowspec = [0.32, 0.19,
                   ['**name', 0.005, 0.0, 0.05],
                   ['PA', 0.005, 0.05, 0.035],
                   ['***stats.pa', 0.055, 0.05, 0.035],
                   ['PD', 0.005, 0.085, 0.035],
...
def redrawstatuswindow(x,y):
    ...
    statuswindow = None
    board.select((x,y))
    if board.getselectedcontents() and board.getselectedcontents().stats['ct'] >= 1.0:
        if board.getselectedcontents().moved:
            statuswindow = gui.newwindow(movedwindowspec, board.getselectedcontents(),
                                         pos, [lambda:board.startattack(), lambda:board.wait()])
        else:
            statuswindow = gui.newwindow(movewindowspec, board.getselectedcontents(),
                                         pos, [lambda:board.startmove(), lambda:board.startattack(), lambda:board.wait()])
    elif board.getselectedcontents():
